  ...
  papermill:
    ...
    steps:
      ...
      - uses: actions/setup-python@v5
        with: { python-version: '3.11' }
      - run: pip install -r requirements.txt
      - name: Execute notebooks with parameters
        run: |
          mkdir -p artifacts
          papermill notebooks/01_bootstrap.ipynb artifacts/01_bootstrap.out.ipynb -p run_mode ci
          papermill notebooks/02_security_supply_chain.ipynb artifacts/02_security_supply_chain.out.ipynb -p run_mode ci
...
5) Create docs/_config.yml, _toc.yml, intro.md per Jupyter Book.
...
1) Templates layout (multi-template repo)
...
2) Minimal cookiecutter.json (Python flavour)
...
3) Hooks (validation + safe bootstrap)
...
if not re.match(r"^[a-z0-9]+(?:-[a-z0-9]+)*$", slug):
    sys.exit("project_slug must be kebab-case: [a-z0-9-].")
...
def rm(*paths):
    for p in paths:
        q = root / p
        if q.exists():
            (q.is_dir() and __import__("shutil").rmtree(q)) or q.unlink()
...
4) CI to test the templates (cookiecutters must render)
...
5) Keep downstream projects in sync (optional but gold)
...
6) Example CI files embedded in templates
...
jobs:
  build:
    ...
    steps:
      ...
      - run: pip install pytest pytest-cov nbmake
      - run: pytest --nbmake notebooks/**/*.ipynb -q
...
8) Tiny Copilot brief (paste into Chat)
...
9) Why these choices (quick evidence)
...
1) package.json (add scripts & devDeps)
