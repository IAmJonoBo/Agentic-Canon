...
SCAFFOLDER_ROOT = Path(__file__).resolve().parents[1]
REPO_ROOT = Path(__file__).resolve().parents[3]
TEMPLATES_DIR = SCAFFOLDER_ROOT / "templates"
...
def _run_sanity_check() -> Tuple[bool, str]:
    ...
    script = REPO_ROOT / ".dev" / "sanity-check.sh"
    if not script.exists():
        return True, "Sanity check script not found; skipped"

    result = _run_command(
        ["bash", str(script), "--quiet", "--skip-templates"], cwd=REPO_ROOT
    )
    if result.returncode == 0:
...
def cmd_repo_init():
    ...
    template_path = TEMPLATES_DIR / "project-management"
