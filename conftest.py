...
_RENDER_ROOT = _REPO_ROOT / "build" / "template-renders"
if _RENDER_ROOT.exists():
    for pyproject in _RENDER_ROOT.rglob("pyproject.toml"):
        src_dir = pyproject.parent / "src"
        if src_dir.is_dir():
            _ensure_sys_path(src_dir)
            _ensure_pythonpath(src_dir)
...
def _patch_hook_environment(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("AGENTIC_CANON_SANITY_MODE", "quick")
